...
/**
 * Class to read data related to Groups
 */

@Component
public class GroupsItemReader implements ItemReader<Document> {
    ...
    public Document read() throws Exception {
        ...
        if (listDocuments.isEmpty()) {
            ...
            ServiceResponseVO serviceResponseVO =
                    webClientWrapper.callGitlabService(pageNo, "/groups", "");
            pageNo++;
            totalPages = serviceResponseVO.getTotalPages();

            if (serviceResponseVO.getResponseJson() == null) {
                return null;
            } else if (serviceResponseVO.getResponseJson().isArray()
